	...
	private void animateZoom(View left, View right, float positionOffset, boolean in) {
		if (mState != State.IDLE) {
			if (left != null) {
				...
				left.setPivotX(left.getMeasuredWidth()*0.1f);
				left.setPivotY(left.getMeasuredHeight()*0.1f);
                Log.i("scaleLeft", mScale + "");
			...
			if (right != null) {
                ...
                Log.i("scaleRight", mScale + "");
				right.setPivotX(right.getMeasuredWidth()*0.1f);
				right.setPivotY(right.getMeasuredHeight()*0.1f);
